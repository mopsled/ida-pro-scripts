...
for function_start in Functions(SegStart(ea), SegEnd(ea)):
	...
	while current_address != BADADDR:
		...
		for ref in code_refs:
			if GetFunctionName(ref) == "__assert":
				functions_with_assert |= {function_name}
				...
				for x in range(10):
					...
					if mnem == "la":
						...
						if register == "$a3":
							string_name = GetOpnd(instruction_address_for_argument, 1)
	...
	for f in functions_with_renames:
		print "Found %s using __assert with function name %s" % (function_name, function_renames[function_name])
	for f in unidentified_funtions:
		print "Found unidentified function %s using assert" % functions_with_renames
